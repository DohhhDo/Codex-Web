...
export const ThemeProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (isDarkMode) {
      ...
      if (themeColorMeta) {
        themeColorMeta.setAttribute('content', '#262624'); // Dark background color (hsl(0 0% 8%))
      }
    } else {
      ...
      const themeColorMeta = document.querySelector('meta[name="theme-color"]');
      if (themeColorMeta) {
        themeColorMeta.setAttribute('content', '#faf9f5'); // Light background color (warm cream)
      }
    }
  }, [isDarkMode]);
