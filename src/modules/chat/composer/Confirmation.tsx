...
const alertVariants = cva(
  'relative grid w-full grid-cols-[0_1fr] items-start gap-y-0.5 rounded-xl border border-border/60 px-3 py-3 text-sm has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] has-[>svg]:gap-x-3 [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current',
  {
    variants: {
      variant: {
...
export const ConfirmationAction: React.FC<ConfirmationActionProps> = ({
  ...
  <Button className="min-h-8 px-3 text-xs" variant={variant} type="button" {...props} />
);
