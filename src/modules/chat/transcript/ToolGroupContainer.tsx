...
function ToolGroupContainer({
  ...
  const ToolIcon = group.toolName === 'Bash' ? TerminalWindowIcon : CodeIcon;
  ...
  return (
    <div className="chat-message tool px-3 sm:px-0" data-message-timestamp={group.timestamp || undefined}>
      <div className="codex-tool-surface">
        <button
          type="button"
          className="codex-tool-heading w-full text-left"
          onClick={() => setIsExpanded((current) => !current)}
          aria-expanded={showChildren}
        >
          <ToolIcon className="h-4 w-4 shrink-0 text-muted-foreground" aria-hidden />
          <span className="min-w-0 max-w-[35%] shrink-0 truncate text-xs font-medium text-foreground">{label}</span>
          ...
          <ChevronRight className={`ml-auto h-3.5 w-3.5 shrink-0 text-muted-foreground transition-transform ${showChildren ? 'rotate-90' : ''}`} aria-hidden />
        </button>

        {showChildren && (
          <div className="codex-tool-group-content">
