...
export default function ProviderSelectionEmptyState({
  ...
  if (!selectedSession && !currentSessionId) {
    return (
      <div className="codex-chat-welcome flex h-full items-center justify-center px-4">
        <div className="w-full max-w-[34.25rem]">
          <div className="codex-welcome-title">
            <CodexWebMark className="text-primary" />
            <h2 className="codex-chat-heading text-foreground">
          ...
          <Dialog open={dialogOpen} onOpenChange={setPickerOpen}>
            <DialogTrigger asChild>
              <button type="button" className="codex-model-choice codex-model-library-link">
                <SquaresFourIcon size={14} />
                <span>{t('composer.modelLibrary', { defaultValue: 'Model library' })}</span>
              </button>
            </DialogTrigger>

            <DialogContent className="max-w-md overflow-hidden p-0">
              <DialogTitle>{t("providerSelection.chooseModel", { defaultValue: "Choose a model" })}</DialogTitle>
