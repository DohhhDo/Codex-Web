...
import { PencilSimpleLineIcon as Edit } from '@phosphor-icons/react/dist/csr/PencilSimpleLine';
...
function getPriorityBadgeClass(priority?: string): string {
  ...
  if (priority === 'low') return 'text-muted-foreground dark:text-muted-foreground bg-accent dark:bg-accent';
  return 'text-muted-foreground dark:text-muted-foreground bg-background dark:bg-secondary';
}
...
export default function TaskDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 md:p-4">
      <div
        className={cn(
          'w-full md:max-w-4xl h-full md:h-[90vh] bg-background border border-border dark:border-border md:rounded-lg shadow-none flex flex-col',
          className,
        )}
      >
        <div className="flex items-center justify-between border-b border-border p-4 dark:border-border md:p-6">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <StatusIcon className="h-6 w-6 text-muted-foreground dark:text-muted-foreground" />
            <div className="min-w-0 flex-1">
              <button
                onClick={() => copyTextToClipboard(String(task.id))}
                className="mb-2 inline-flex items-center gap-1 rounded bg-secondary px-2 py-1 text-xs text-muted-foreground hover:bg-accent dark:bg-secondary dark:text-muted-foreground dark:hover:bg-accent"
                title="Copy task ID"
              >
              ...
              {isEditMode ? (
                <input
                  ...
                  className="w-full border-b-2 border-border bg-transparent text-lg font-semibold text-foreground focus:outline-none dark:text-foreground"
                />
              ) : (
                <h1 className="line-clamp-2 text-lg font-semibold text-foreground dark:text-foreground md:text-xl">{task.title}</h1>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {isEditMode ? (
              <>
                ...
                <button
                  ...
                  disabled={isSaving}
                  className="rounded-md p-2 text-muted-foreground hover:bg-secondary dark:hover:bg-secondary"
            ...
            <button onClick={onClose} className="rounded-md p-2 text-muted-foreground hover:bg-secondary dark:hover:bg-secondary" title="Close">
              <X className="h-5 w-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-4 md:p-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground dark:text-muted-foreground">Status</label>
              <select
                value={task.status ?? 'pending'}
                onChange={(event) => {
                  void handleStatusSelect(event.target.value);
                }}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-foreground dark:border-border dark:bg-secondary dark:text-foreground"
              >
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground dark:text-muted-foreground">Priority</label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground dark:text-muted-foreground">Dependencies</label>
                ...
                <div className="flex flex-wrap gap-1">
                  {task.dependencies.map((dependency) => (
                    <button
                      ...
                      className="rounded bg-accent px-2 py-1 text-sm text-muted-foreground hover:bg-accent dark:bg-accent dark:text-muted-foreground dark:hover:bg-accent"
                    >
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-foreground dark:text-muted-foreground">Description</label>
            {isEditMode ? (
              <textarea
                rows={4}
                value={editableTask.description ?? ''}
                onChange={(event) => setEditableTask({ ...editableTask, description: event.target.value })}
                className="w-full rounded-md border border-border bg-background px-3 py-2 dark:border-border dark:bg-secondary"
              />
            ) : (
              <p className="whitespace-pre-wrap text-foreground dark:text-muted-foreground">{task.description || 'No description provided'}</p>
            )}
          </div>
          ...
          {task.details && (
            <div className="rounded-lg border border-border dark:border-border">
              <button
                ...
                className="flex w-full items-center justify-between p-4 text-left hover:bg-background dark:hover:bg-secondary"
              >
                <span className="text-sm font-medium text-foreground dark:text-muted-foreground">Implementation Details</span>
                {showDetails ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
              </button>
              {showDetails && (
                <div className="border-t border-border p-4 dark:border-border">
                  <p className="whitespace-pre-wrap text-foreground dark:text-muted-foreground">{task.details}</p>
                </div>
              )}
            </div>
          )}
          ...
          {task.testStrategy && (
            <div className="rounded-lg border border-border dark:border-border">
              <button
                ...
                className="flex w-full items-center justify-between p-4 text-left hover:bg-background dark:hover:bg-secondary"
              >
                <span className="text-sm font-medium text-foreground dark:text-muted-foreground">Test Strategy</span>
                {showTestStrategy ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
              </button>
              {showTestStrategy && (
                <div className="border-t border-border bg-accent p-4 dark:border-border dark:bg-accent/30">
                  <p className="whitespace-pre-wrap text-foreground dark:text-muted-foreground">{task.testStrategy}</p>
                </div>
              )}
            </div>
          )}
