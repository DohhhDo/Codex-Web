...
export class CodexProviderAuth implements IProviderAuth {
  ...
  async getStatus(): Promise<ProviderAuthStatus> {
    ...
    return {
      ...
      subscriptionLevel: credentials.subscriptionLevel,
    ...
    try {
      ...
      if (idToken || accessToken) {
        return {
          ...
          subscriptionLevel: idToken ? this.readSubscriptionFromIdToken(idToken) : undefined,
        };
      }
  ...
  /** Reads only the optional display claim; never returns the credential itself. */
  private readSubscriptionFromIdToken(idToken: string): string | undefined {
    try {
      const payload = readObjectRecord(JSON.parse(Buffer.from(idToken.split('.')[1], 'base64url').toString('utf8')));
      const account = readObjectRecord(payload?.['https://api.openai.com/auth']);
      return readOptionalString(account?.chatgpt_plan_type) ?? undefined;
    } catch {
      return undefined;
    }
  }
