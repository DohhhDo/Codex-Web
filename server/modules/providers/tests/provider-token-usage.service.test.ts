...
import { readCodexUsageMetadata } from '@/shared/index.js';
...
test('Codex token usage uses the latest token_count snapshot', async () => {
  ...
  try {
    await writeFile(sessionFilePath, [
      ...
      JSON.stringify({
        ...
        payload: {
          type: 'token_count',
          info: {
            total_token_usage: { input_tokens: 40, cached_input_tokens: 20, output_tokens: 9, total_tokens: 49 },
            model_context_window: 250_000,
          },
          rate_limits: { plan_type: "plus", primary: { used_percent: 12, window_minutes: 300, resets_at: 1900000000 } },
        },
      }),
    ].join('\n'));
    ...
    assert.deepEqual(await service.getSessionTokenUsage('app-session'), {
      ...
      rateLimits: { primary: { usedPercent: 12, windowMinutes: 300, resetsAt: 1900000000 } },
      used: 49,
      total: 250_000,
      inputTokens: 40,
      outputTokens: 9,
...
test('Codex display metadata preserves zero cache and omits unknown or invalid limits', () => {
  assert.deepEqual(readCodexUsageMetadata({ cached_input_tokens: 0 }, null), { cacheReadTokens: 0 });
  assert.deepEqual(readCodexUsageMetadata({ model_context_window: 200000 }, {
    primary: { used_percent: '12' }, secondary: { used_percent: -1 },
  }), {});
  assert.deepEqual(readCodexUsageMetadata({ cached_input_tokens: NaN }, {
    primary: { used_percent: 0, window_minutes: 300, secret: 'never forward' },
    secondary: { used_percent: 25, window_minutes: 10080 }, credits: { balance: 100 },
  }), { rateLimits: { primary: { usedPercent: 0, windowMinutes: 300 }, secondary: { usedPercent: 25, windowMinutes: 10080 } } });
});
