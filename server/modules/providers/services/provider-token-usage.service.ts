...
import type { CodexUsageMetadata } from '@/shared/index.js';
...
type TokenUsageResult = CodexUsageMetadata & {
  used: number;
  total?: number;
  inputTokens: number;
  outputTokens: number;
...
function findCodexTokenUsage(fileContent: string): TokenUsageResult | null {
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    try {
      ...
      return {
        ...readCodexUsageMetadata(tokenInfo.total_token_usage, entry.payload.rate_limits),
        used: totalTokens,
