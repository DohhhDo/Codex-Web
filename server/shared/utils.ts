...
//----------------- PROVIDER USAGE DISPLAY METADATA ------------
/** Used by Codex realtime, history and usage readers. Allowlist only display
 * fields from the CLI snapshot; missing numbers stay unknown, including cache
 * counters, and no context-window value is treated as an account allowance. */
export function readCodexUsageMetadata(usageValue: unknown, limitsValue: unknown): CodexUsageMetadata {
  const usage = readObjectRecord(usageValue);
  const limits = readObjectRecord(limitsValue);
  const result: CodexUsageMetadata = {};
  const nonnegative = (value: unknown): value is number => typeof value === 'number' && Number.isFinite(value) && value >= 0;
  if (nonnegative(usage?.cached_input_tokens)) result.cacheReadTokens = usage.cached_input_tokens;
  const plan = readOptionalString(limits?.plan_type);
  if (plan) result.subscriptionLevel = plan;
  for (const key of ['primary', 'secondary'] as const) {
    const window = readObjectRecord(limits?.[key]);
    if (!nonnegative(window?.used_percent)) continue;
    result.rateLimits ??= {};
    result.rateLimits[key] = {
      usedPercent: window.used_percent,
      ...(nonnegative(window.window_minutes) ? { windowMinutes: window.window_minutes } : {}),
      ...(nonnegative(window.resets_at) ? { resetsAt: window.resets_at } : {}),
    };
  }
  return result;
}
