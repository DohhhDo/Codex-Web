...
- **设置个人资料**：优先使用 Codex 授权中可用的名称和头像，也可在设置中修改名称、上传头像或恢复自动资料。
...
打开 **http://localhost:3001**，直接选择工程开始使用，无需注册或登录本站。Codex 授权仍用于连接模型服务。
...
Codex-Web 采用单工作区直接访问：能够访问服务地址的人可以使用同一工作区。已有本地用户记录继续承载设置、草稿等数据；新安装会自动建立内部归属记录，不需要密码。

在「设置 → 个人资料」中可修改显示名称、上传 PNG／JPG／WebP 头像（最大 1 MB）。资料保存在服务端，刷新和重启后仍保留。Codex 授权未提供名称或头像时使用默认值，手动设置优先；这些更改不会修改 Codex 账号。首次使用引导也可从这里按需打开。
