// Service Worker for Codex-Web PWA
// Cache only manifest (needed for PWA install). HTML and JS are never pre-cached
// so a rebuild + refresh always picks up the latest assets.
const CACHE_NAME = 'codex-web-f70d154b99';
const urlsToCache = [
  '/manifest.json'
];
...
self.addEventListener('push', event => {
  ...
  try {
    ...
    payload = { title: 'Codex-Web', body: event.data.text() };
  }

  const options = {
    body: payload.body || '',
    icon: '/icons/codex-web-f70d154b99-256.png',
    badge: '/icons/codex-web-f70d154b99-128.png',
    data: payload.data || {},
  ...
  event.waitUntil(
    self.registration.showNotification(payload.title || 'Codex-Web', options)
  );
});
